<template>
  <div class="mx-auto">
    ...
    <div>
      <form @submit.prevent="submitForm" class="flex gap-4 flex-col">
        <div class="field">
          <label>Email</label>
        ...
        <div class="field items-end">
          <button class="button font-bold bg-slate-500 text-white hover:bg-slate-400">Submit</button>
        </div>
      </form>
    </div>
  </div>
...
const submitForm = () => {
  errors.value = []
  if (email.value === '') { errors.value.push('請輸入Email') }
...
<style lang="postcss" scoped>
  ...
  .error-text {
    @apply text-rose-600 bg-rose-200 px-4 py-2 rounded-lg text-center
  }
</style>
